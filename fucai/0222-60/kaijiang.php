...
foreach($data as $row){
	...
	echo "<tr><td>$row[1]</td><td style='color:red'>$row[2]</td><td style='color:red'>$row[3]</td><td style='color:red'>$row[4]</td><td>$row[5]</td><td>$row[6]</td><td>$row[7]</td><td>$row[8]</td><td>$row[9]</td></tr>";
}
echo '</tr>';
...
for($index=0;$index<$dataNum;$index++){
    ...
    for($j=1;$j<=20;$j++) {
        ...
        if($isLLian){
            $style.='font-size:16px;background:#bbb;';
        }
        if($row[$j]==0){
            echo "<td></td>";
        }else{
