...
$sql ="select date,num_1,num_2,num_3,num_4,num_5 from ticai order by date desc  limit 71";
...
for($index=0;$index<$dataNum;$index++){
	...
	$isSingle = true;//全单，为true为真
	$isDouble = true; //全双，为true为真
    ...
    for($j=1;$j<=11;$j++) {
        ...
        $style = '';
        if($isHLian){
            $style.='background-color:#777;font-size:16px;';
        }
        ...
        if($isLLian){
            $style.='font-size:16px;background-color:#999;';
        }
        if($row[$j]==0){
            echo "<td><p></p></td>";
        }else{
